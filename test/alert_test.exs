defmodule TodoOrDie.AlertTest do
  ...
  test "Date" do
    ...
    assert Alert.message(item, %{current_datetime: parse_datetime("2021-02-14 12:00")}, options) == nil
  ...
  test "DateTime" do
    ...
    assert Alert.message(item, %{current_datetime: parse_datetime("2021-02-14 12:33")}, options) == nil
  ...
  describe "Package" do
    ...
    test ">= is used", %{context: context} do
      ...
      |> Alert.message(context, %{})
      assert(message == "Found a TOTEST tag: Test String (some_package requirement `>=1.1.4` matched, current version: 1.2.3)")
  ...
  describe "GitHub issue" do
    test "Should alert when an issue has been closed" do
      HTTPoison.Base.Mock
      |> expect(:get, fn
        "https://api.github.com/repos/user123/repo321/issues/1234" -> {:ok, %HTTPoison.Response{body: ~s({"state":"closed"}), status_code: 200}}
      end)

      message =
        %Item{expression: "user123/repo321#1234", tag: "TOTEST", string: "Test String"}
        |> Alert.message(%{httpoison_module: HTTPoison.Base.Mock}, %{})
      assert(message == "Found a TOTEST tag: Test String (issue #1234 in repo user123/repo321 has been closed)")
    end

    test "Should not alert when an issue is open" do
      HTTPoison.Base.Mock
      |> expect(:get, fn
        "https://api.github.com/repos/user123/repo321/issues/1234" -> {:ok, %HTTPoison.Response{body: ~s({"state":"open"}), status_code: 200}}
      end)
    ...
    test "Should deal with non-200 response" do
      HTTPoison.Base.Mock
      |> expect(:get, fn
        "https://api.github.com/repos/user123/repo321/issues/1234" -> {:ok, %HTTPoison.Response{body: ~s({"error":"dunno"}), status_code: 500}}
      end)

      message =
        %Item{expression: "user123/repo321#1234", tag: "TOTEST", string: "Test String"}
        |> Alert.message(%{httpoison_module: HTTPoison.Base.Mock}, %{})
      assert(message == "Found a TOTEST tag: Test String (WAS UNABLE TO CHECK THE ISSUE - HTTP STATUS 500)")
    end

    test "Should deal with connection errors" do
      HTTPoison.Base.Mock
      |> expect(:get, fn
        "https://api.github.com/repos/user123/repo321/issues/1234" -> {:error, %HTTPoison.Error{id: nil, reason: {:tls_alert, 'internal error'}}}
      end)

      message =
        %Item{expression: "user123/repo321#1234", tag: "TOTEST", string: "Test String"}
        |> Alert.message(%{httpoison_module: HTTPoison.Base.Mock}, %{})
      assert(message == "Found a TOTEST tag: Test String (WAS UNABLE TO CHECK THE ISSUE - GOT ERROR: `{:tls_alert, 'internal error'}`)")
    end
  end
end
